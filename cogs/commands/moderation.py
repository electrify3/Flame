...
class Moderation(commands.Cog):
	...
	async def mute(self, ctx, members: commands.Greedy[discord.Member], time="1h", *, reason="No reason specified."):
		...
		if not members: return await ctx.send(f"{self.client.warning} | Provide atleast one @user/id.")
		elif len(members) > 10 and ctx.author != ctx.guild.owner: return await ctx.send(f"{self.client.fail} | You can use this command on more than `10` users.")
		...
		elif time.total_seconds() > 60*60*24*28: return await ctx.send(f"{self.client.fail} | Timeout can't be more than 28days.")
		for member in members:
			if ctx.author.top_role.position <= member.top_role.position and ctx.author != ctx.guild.owner:
				await ctx.send(f"{self.client.fail} | You can't mute `{member}`.")
				continue
			if member.is_timed_out():
				await ctx.send(f"{self.client.warning} | `{member}` is already muted.")
				continue
			try:
				await member.timeout(time, reason=f"{ctx.author}: {reason}")
				await ctx.send(f"{self.client.success} | Successfully muted `{member}` till <t:{int(time.total_seconds() + datetime.datetime.utcnow().timestamp())}>.")
			except:
				await ctx.send(f"{self.client.fail} | Unable to mute `{member}`.")
	...
	async def unmute(self, ctx, members: commands.Greedy[discord.Member],*, reason="No reason specified."):
		...
		for member in members:
			if not member.is_timed_out():
				await ctx.send(f"{self.client.fail} | `{member}` is not muted.")
				continue
			await member.timeout(None, reason=f"{ctx.author}: {reason}")
			await ctx.send(f"{self.client.success} | Successfully unmuted `{member}`.")
	...
	async def kick(self, ctx, members: commands.Greedy[discord.Member], *, reason="No reason specified."):
		...
		for member in members:
			if ctx.author.top_role <= member.top_role and ctx.author != ctx.guild.owner:
				await ctx.send(f"{self.client.fail} | You don't have permission to kick `{member}`.")
				continue
			if member.top_role >= ctx.guild.me.top_role or ctx.guild.owner is member:
				await ctx.send(f"{self.client.fail} | I don't have permission to kick `{member}`.")
				continue
			
			try:
				await member.kick(reason=f"{ctx.author}: {reason}")
				await ctx.send(f"{self.client.success} | Successfully kicked `{member}`.")
			except:
				await ctx.send(f"{self.client.fail} | I am unable to kick `{member}`.")
	...
	async def ban(self, ctx, users: commands.Greedy[discord.User], days: typing.Optional[int] = 1, *, reason="No reason specified."):
		...
		if days > 7:
			days = 7
			await ctx.send(f"{self.client.warning} | Message deletion days can't be more than `7` days, falling back to max allowed value `7`.")
		if not users: return await ctx.send(f"{self.client.warning} | Provide atleast one @user/id.")
		elif len(users) > 10 and ctx.author != ctx.guild.owner: return await ctx.send(f"{self.client.fail} | You can't use this command on more than `10` users.")
		...
		for user in users:
			try:
				...
				if ctx.author.top_role <= user.top_role and ctx.author != ctx.guild.owner:
					await ctx.send(f"{self.client.fail} | You don't have permission to ban `{user}`.")
					continue
				if user.top_role >= ctx.guild.me.top_role or ctx.guild.owner is user:
					await ctx.send(f"{self.client.fail} | I don't have permission to ban `{user}`.")
					continue
			except:
				pass
			
			if user.id in bans:
				await ctx.send(f"{self.client.fail} | `{user}` is already banned.")
				continue
			
			try:
				await ctx.guild.ban(user, reason=f"{ctx.author}: {reason}", delete_message_days=days)
				await ctx.send(f"{self.client.success} | Successfully banned `{user}`.")
			except:
				await ctx.send(f"{self.client.fail} | I am unable to ban `{user}`.")
	...
	async def unban(self, ctx, users: commands.Greedy[discord.User], *, reason="No reason specified."):
		...
		if not users: return await ctx.send(f"{self.client.warning} | Provide atleast one @user/id.")
		elif len(users) > 10 and ctx.author != ctx.guild.owner: return await ctx.send(f"{self.client.fail} | You can use this command on more than `10` users.")
		
		bans = [entry.user.id async for entry in ctx.guild.bans(limit=None)]
		
		for user in users:
			if not user.id in bans:
				await ctx.send(f"{self.client.fail} | `{user}` is not banned.")
				continue
			await ctx.guild.unban(user ,reason =f"{ctx.author}: {reason}")
			await ctx.send(f"{self.client.success} | Successfully unbanned `{user}`.")
	
	
	@commands.hybrid_command(name="unbanall", description="Unbans all banned users.", aliases=["uball"], usage="unbanall")
	async def unbanall(self, ctx):
		if ctx.guild.owner != ctx.author:
			await ctx.send(f"{self.client.fail} | Only server owner can use this command!")
			return
		banned_users = [ban.user async for ban in ctx.guild.bans(limit=None)]
		message = await ctx.send(f"{self.client.working} | Removing bans from **{len(banned_users)}** Users, please be patient.")
		for user in banned_users:
			await ctx.guild.unban(user ,reason =f"{ctx.author} used unbanall command.")
		await message.edit(content=f"{self.client.success} | Successfully unbanned **{len(banned_users)}** users.")
	...
	async def warn(self, ctx, member : discord.Member,*, reason="[No reason specified]"):
		if member.bot: return await ctx.send(f"{self.client.fail} | You can't warn a bot!")
		elif member == ctx.author: return await ctx.send(f"{self.client.fail} | You can't warn yourself!")
		elif ctx.author.top_role <= member.top_role and ctx.author != ctx.guild.owner: return await ctx.send(f"{self.client.fail} | You can't warn `{member}`.")
		elif member == ctx.guild.owner: return await ctx.send(f"{self.client.fail} | You can't warn owner.")
	...
	async def purge(self, ctx, limit: int):
		if limit > 1000: return await ctx.send(f"{self.client.fail} | You can't delete more than **1000** messages at once.")
		...
		if len(purged) == 0: await ctx.send(f"{self.client.fail} | No message found!")
		else: await ctx.send(f"{self.client.success} | Successfully deleted **{len(purged)}** messages.", delete_after=5)
	...
	async def user(self, ctx, users: commands.Greedy[discord.User], limit: int = 100):
		if limit > 1000: return await ctx.send(f"{self.client.fail} | You can't delete more than **1000** messages at once.")
		users = [*set(users)]
		if not users: return await ctx.send(f"{self.client.fail} | Invalid command usage, `users` are not specified!")
	...
	async def content(self, ctx, *, text: str, limit: int = 100):
		...
		if limit > 1000: return await ctx.send(f"{self.client.fail} | You can't delete more than **1000** messages at once.")
		if not text: return await ctx.send(f"{self.client.fail} | Invalid command usage, `content` is not specified!")
