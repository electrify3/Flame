...
class Admin(commands.Cog):
	...
	@admin.command(aliases=["wk", "vickeybhedia", "meaning"])
	async def wikipedia(self, ctx,*, query):
		try :
			re = wk.summary(query, sentences=5)
		except Exception as e:
			re = e
		em = discord.Embed(title=query, description=re, color=discord.Colour.yellow())
		await ctx.message.reply("Wikipedia", embed=em, mention_author=False)
