...
class Music(commands.Cog):
    ...
    async def nowplaying(self, ctx):
        player = self.bot.lavalink.player_manager.get(ctx.guild.id)
        if not player.is_playing:
            return await ctx.send("{self.bot.warning} | Currently no song is playing.")
    ...
    async def volume(self, ctx: commands.Context, value: int = None):
        ...
        if value < 0 or value > 1000:
            return await ctx.reply("{self.bot.warning} | Volume can't be more then 1000 and less then 0.", delete_after=5)
        
        await player.set_volume(value)
        await ctx.send(f'<:Esuccess:1134146893383991466> | Successfully set volume to {value}.')
    ...
    async def speed(self, ctx: commands.Context, value: float = None):
        ...
        filter = player.get_filter('timescale')
        if value is None:
            if filter:
                speed = filter.serialize()['speed']
        ...
        if value < 0.1 or value > 5:
            return await ctx.reply("{self.bot.warning} | Speed can't be more then 5x and less then 0.1x.", delete_after=5)
        
        if filter:
            filter.update(speed=value)
        else:
            filter = lavalink.filters.Timescale(speed=value)
