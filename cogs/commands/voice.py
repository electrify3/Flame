...
class Voice(commands.Cog):
	...
	@voice.command(name="kick", description="Disconnect's a user(s) from a voice channel.", usage="voice kick <@users/ids>")
	...
	@voice.command(name="mute", description="Voice mutes a member.", usage="voice mute <@users/ids>")
	...
	@voice.command(name="deafen", description="Deafen a member.", usage="voice deafen <@users/id>")
	...
	async def deafen(self, ctx, members: commands.Greedy[discord.Member]):
		...
		for member in members:
			...
			if member.voice.deaf:
				...
				continue
			await member.edit(deaf=True, reason=f"{member}: Used voice deafen command.")
			await ctx.send(f"{self.client.success} | Successfully voice deafened `{member}`.")
	
	
	@voice.command(name="undeafen", description="Undeafen a member.", usage="voice undeafen <@members/ids>")
	@commands.has_guild_permissions(deafen_members=True)
	...
	async def undeafen(self, ctx, members: commands.Greedy[discord.Member]):
		...
		for member in members:
			...
			if not member.voice.deaf:
				...
				continue
			await member.edit(deaf=False, reason=f"{member}: Used voice undeaf command.")
